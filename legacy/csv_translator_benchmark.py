...
class CSVTranslator:
    ...
    def translate_text(self, text: str, is_warmup: bool = False) -> str:
        """
        ...
        Args:
            ...
            is_warmup: Whether this is a warmup translation (for benchmarking)
        ...
        try:
            ...
            # Update character count for benchmarking (but not for warmup)
            if not is_warmup:
                self._update_character_count(text_str)
            ...
            # Update translated character count for benchmarking (but not for warmup)
            if not is_warmup:
                self._update_character_count(translated, is_translated=True)
    ...
    def translate_csv(self, 
        ...
        try:
            ...
            # Calculate final runtime for benchmarking
            if self.benchmark_data['start_time'] is not None:
                self.benchmark_data['total_runtime'] = time.time() - self.benchmark_data['start_time']
    ...
    def _warmup_translator(self):
        """
        Warm up the translator with a simple translation to initialize connections.
        Times the warmup process for benchmarking.
        """
        logger.info("Warming up translator...")
        warmup_text = "test"
        
        self.benchmark_data['warmup_start_time'] = time.time()
        
        try:
            # Perform warmup translation
            _ = self.translate_text(warmup_text, is_warmup=True)
            self.benchmark_data['warmup_completed'] = True
            self.benchmark_data['warmup_time'] = time.time() - self.benchmark_data['warmup_start_time']
            logger.info(f"Translator warmed up successfully in {self.benchmark_data['warmup_time']:.2f} seconds")
        except Exception as e:
            logger.warning(f"Warmup failed: {e}")
            self.benchmark_data['warmup_time'] = time.time() - self.benchmark_data['warmup_start_time']
            self.benchmark_data['warmup_completed'] = False
    
    def get_benchmark_summary(self) -> Dict[str, any]:
        """
        Get a summary of benchmarking data.
        
        Returns:
            Dictionary containing benchmark metrics
        """
        if self.benchmark_data['total_runtime'] > 0 and self.benchmark_data['translated_characters'] > 0:
            chars_per_second = self.benchmark_data['translated_characters'] / self.benchmark_data['total_runtime']
        else:
            chars_per_second = 0.0
            
        return {
            'warmup_time': self.benchmark_data['warmup_time'],
            'warmup_completed': self.benchmark_data['warmup_completed'],
            'total_runtime': self.benchmark_data['total_runtime'],
            'total_characters': self.benchmark_data['total_characters'],
            'translated_characters': self.benchmark_data['translated_characters'],
            'characters_per_second': chars_per_second
        }
    
    def print_benchmark_summary(self):
        """Print a formatted summary of benchmarking data."""
        summary = self.get_benchmark_summary()
        
        print("\n" + "="*50)
        print("TRANSLATION BENCHMARK SUMMARY")
        print("="*50)
        print(f"Warmup Time:           {summary['warmup_time']:.3f} seconds")
        print(f"Warmup Successful:     {'Yes' if summary['warmup_completed'] else 'No'}")
        print(f"Total Runtime:         {summary['total_runtime']:.3f} seconds")
        print(f"Total Characters:      {summary['total_characters']:,}")
        print(f"Translated Characters: {summary['translated_characters']:,}")
        print(f"Translation Speed:     {summary['characters_per_second']:.1f} chars/second")
        print("="*50)
    
    def _start_timing(self):
        """Start timing for the main translation process."""
        if self.benchmark_data['start_time'] is None:
            self.benchmark_data['start_time'] = time.time()
    
    def _update_character_count(self, text: str, is_translated: bool = False):
        """Update character count for benchmarking."""
        char_count = len(text) if text else 0
        self.benchmark_data['total_characters'] += char_count
        if is_translated:
            self.benchmark_data['translated_characters'] += char_count
    ...
    while True:
        try:
            ...
            if target_choice.strip() == "":
                target_lang, target_name = lang_options[0]
                print(f"Selected: {target_name} (default)")
                break
...
def process_batch_files(params: Dict[str, any]) -> bool:
    ...
    for file_path in discovered['root_files']:
        current_file += 1
        print(f"📄 Processing file {current_file}/{total_files}: {file_path.name}")
    ...
    for folder_name, files in discovered['folders'].items():
        ...
        print(f"📁 Created target folder: {target_subfolder}")
        
        for file_path in files:
            current_file += 1
            relative_path = file_path.relative_to(SOURCE_DIR)
            print(f"📄 Processing file {current_file}/{total_files}: {relative_path}")
    ...
    if successful_files > 0:
        print(f"\n✅ Successfully translated files are saved in: {TARGET_DIR}")
        print(f"📋 Check the log file for detailed information: translation.log")
