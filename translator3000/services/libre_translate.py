...
This module provides LibreTranslate API integration with self-hosted server detection
and automatic URL selection for optimal performance.
"""
...
class LibreTranslateService(BaseTranslationService):
    """LibreTranslate API service wrapper with self-hosted detection."""
    
    def __init__(self, source_lang: str, target_lang: str, api_url: str = None, api_key: str = ""):
        super().__init__(source_lang, target_lang)
        self.api_key = api_key
        
        # Choose URL based on selfhost availability if not specified
        if api_url is None:
            config = get_config()
    ...
    def translate(self, text: str) -> Optional[str]:
        ...
        try:
            ...
            if "Rate limit" in str(e) or "429" in str(e):
                ...
                logger.error("LibreTranslate requires requests library")
            else:
